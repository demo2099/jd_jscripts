...
await $.get({url:"http://jdhelper.tk/ddfactory/"+${item.assistTaskDetailVo.taskToken}+"?ti="+Date.now()},(err,resp,data)=>{try{if(err){console.log("查询jdShareCode: API查询请求失败 ‼️‼️");$.logErr(err)}else{console.log("【查询jdShareCode】"+resp.body)}}catch(e){$.logErr(e,resp)}});
                }
              })
            }
          }
